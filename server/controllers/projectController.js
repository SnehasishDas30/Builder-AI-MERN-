...
async function runBackgroundGeneration(projectId, prompt) {
    ...
    try {
        ...
        const result = await generateProject(prompt, {
            ...
            onFileComplete: async (path, code) => {
                ...
                if (project) {
                    ...
                    const generated = Array.isArray(project.filesGenerated)
                    ...
                    project.filesGenerated = generated;
        ...
        if (project) {
            ...
            project.messages.push({
                role: "assistant",
                content: `Website generation complete! You can view and
edit the files.`,
                timestamp: new Date(),
            })
            await project.save();
        ...
        console.error(`[Background AI] Fatal generation error for project $
{projectId}:`, err);
await Project.findByIdAndUpdate(projectId, {
    status: "failed",
    error: err.message,
...
export async function updateProjectFiles(req, res) {
    ...
    if (!validation.valid) {
        res.status(400).json({
            error: "Project contains invalid code.",
