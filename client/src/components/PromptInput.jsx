...
const recognitionRef = useRef(null);
    ...
    if (variant === "glass") {
        return (
            <form
                ...
                className='max-w-2xl w-full bg-white/10 backdrop-blur-xl
            ...
            >
                <textarea
                    ...
                    disabled={loading}
                    rows={3}
                    className='w-full p-4 pb-2 resize-none
                placeholder:text-white/60 outline-none bg-transparent text-white text-base'
                />
    ...
    disabled={loading}
    className={`flex items-center justify-center p-1 cursor-pointer transition ${
        isListening
            ? "text-red-400"
    ...
    return (
        <div className={`bg-white border border-zinc-200 rounded-xl flex items-end gap-2 focus-within:ring-1 focus-within:ring-zinc-300 transition ${large ? "p-4" : "p-3"}`}>
            ...
            <textarea ref={textareaRef}
                ...
                disabled={loading}
                rows={large ? 5 : 1}
                className={`flex-1 bg-transparent border-none outline-none resize-none text-zinc-800 placeholder:text-zinc-400 ${large ? "text-base" : "text-sm"}`} />
